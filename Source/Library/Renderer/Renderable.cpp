...
namespace library
{
    ...
    HRESULT Renderable::initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext) {
        ...
        hr = pDevice->CreateBuffer(&bd, &InitData, GetVertexBuffer().GetAddressOf());

        if (FAILED(hr)) return hr;

        UINT stride = sizeof(SimpleVertex);
        UINT offset = 0;
        pImmediateContext->IASetVertexBuffers(0, 1, GetVertexBuffer().GetAddressOf(), &stride, &offset);

        // Create & Set Index Buffer

        bd =
        {
            .ByteWidth = sizeof(WORD)*36,
            .Usage = D3D11_USAGE_DEFAULT,
        ...
        hr = pDevice->CreateBuffer(&bd, &InitData, GetIndexBuffer().GetAddressOf());

        if (FAILED(hr)) return hr;

        pImmediateContext->IASetIndexBuffer(GetIndexBuffer().Get(), DXGI_FORMAT_R16_UINT, 0);

        //Create & set Constant Buffer
        ...
        hr = pDevice->CreateBuffer(&bd, nullptr, GetConstantBuffer().GetAddressOf());
    ...
    {
        m_world = XMMatrixRotationX(angle) * m_world;
    ...
    {
        m_world = XMMatrixRotationY(angle) * m_world;
    ...
    {
        m_world = XMMatrixRotationZ(angle) * m_world;
    ...
    {
        m_world = XMMatrixRotationRollPitchYaw(pitch, yaw, roll) * m_world;
    ...
    {
        m_world = XMMatrixTranslationFromVector(offset) * m_world;
